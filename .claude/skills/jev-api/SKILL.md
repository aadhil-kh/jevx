...
- **AI slop** (`ai_slop`, in both requests, added in schema v5): one `score` question over `SLOP_LEVELS` — 6 ascending rubric levels from `slop` to `distinctive`, so the raw score runs 0–5. It rides along in calls that are sent anyway (speculative fan-out), never its own request. Both content scripts rescale it identically: `value = round(score / SLOP_TOP_LEVEL * 10)` → 0–10, banded 0–4 `low` (red), 5–6 `mid` (amber), 7–10 `high` (green), and hidden entirely below `SLOP_MIN_CONFIDENCE` (0.45). `slopDisplay()`, `SLOP_BANDS` and `SLOP_LEVELS` are duplicated byte-for-byte in `src/content.js` and `src/timeline.js` (the test checks). The popup's `slopEnabled` switch hides the pill only; it never changes a request or a cache key. Word it as a judgment of the writing, never as AI detection.
